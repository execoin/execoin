...
uint256 hashGenesisBlock("0xb9429871ab336e52be203367d341133d03e446c7e05be3ff3179bdaa797b2c71");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Execoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1390959880;
int64 nChangeInterval = 47335389;
...
const string strMessageMagic = "Execoin Signed Message:\n";
...
{
    // Execoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Execoin: 1 day
static const int64 nTargetSpacing = 45; // Execoin: 45 seconds
...
unsigned int static GetNextWorkRequired_legacy(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Execoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Execoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Execoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 'e';
        pchMessageStart[3] = 'c';
        hashGenesisBlock = uint256("0xe6030581406dc03ef244e8408f07ec84a56e9e5aef54127738e7ddb231a0fa10");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "JAN. 28, 2014 Pete Seeger, Songwriter and Champion of Folk Music, Dies at 94";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1390959880;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084845118;

        if (fTestNet)
        {
            block.nTime    = 1390958000;
            block.nNonce   = 389026157;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd6d9cb74b3954c44a6a81585d04ef24752cd8b5689af63f58093581cfade05b9"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Execoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ExecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ExecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ExecoinMiner(CWallet *pwallet)
{
    printf("ExecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("execoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running ExecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ExecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ExecoinMiner, pwallet));
}
